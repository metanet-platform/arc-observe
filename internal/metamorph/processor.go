...
func (p *Processor) statusUpdateWithCallback(ctx context.Context, statusUpdates, doubleSpendUpdates []store.UpdateStatus) (err error) {
	...
	var updatedData []*store.TransactionData
...
func (p *Processor) registerTransactions(ctx context.Context, data []*store.TransactionData) error {
...
func (p *Processor) ProcessTransaction(ctx context.Context, req *ProcessorRequest) {
	...
	// BROADCASTING DISABLED: Skip asking network and announcing transaction
	// p.bcMediator.AskForTxAsync(ctx, req.Data.Hash)
	// p.bcMediator.AnnounceTxAsync(ctx, req.Data.Hash, req.Data.RawTx)
	p.logger.Debug("Broadcasting skipped - consider transaction announced", slog.String("hash", req.Data.Hash.String()))
...
func (p *Processor) ProcessTransactions(ctx context.Context, sReq []*store.TransactionData) {
	var err error
	...
	for _, data := range sReq {
		...
		// BROADCASTING DISABLED: Skip announcing transaction to network
		// p.bcMediator.AnnounceTxAsync(ctx, data.Hash, data.RawTx)
		p.logger.Debug("Broadcasting skipped - consider transaction announced", slog.String("hash", data.Hash.String()))
...
func (p *Processor) storeData(ctx context.Context, data *store.TransactionData) error {
	data.LastSubmittedAt = p.now()
	return p.store.Set(ctx, data)
}

func addNewCallback(data, reqData *store.TransactionData) {
...
func callbackExists(callback store.Callback, data *store.TransactionData) bool {
